...
const WINDOWS_TRUSTED_ENVIRONMENT_NAMES = new Set([
  "systemroot",
  "windir",
  "comspec",
]);
...
interface Schema9RuntimeCommandLaunch {
...
async function runCommand(
  ...
  useWindowsSupervisor = true,
): Promise<string> {
  throwIfAborted(signal);
  const launch = await prepareSchema9RuntimeCommandLaunch(
    executable,
    arguments_,
    cwd,
    environment,
    useWindowsSupervisor,
  );
  throwIfAborted(signal);
  return new Promise((accept, reject) => {
    ...
    const child = spawn(
      ...
      [...launch.arguments],
      {
        ...
        stdio: [
          launch.standardInput === undefined ? "ignore" : "pipe",
          "pipe",
          "pipe",
        ],
      },
    ...
    if (childStdout === null || childStderr === null) {
      child.kill("SIGKILL");
      reject(new Error("Schema-9 runtime command output pipes were not created."));
      return;
    }
    ...
    const settle = async (
      ...
      if (launch.authenticatedLauncher !== undefined) {
        try {
          ...
          terminationFailure = combineSchema9CleanupFailures(
            terminationFailure,
            error,
            "Schema-9 runtime launcher and cleanup verification both failed.",
          );
        }
      }
      if (signal?.aborted === true) {
      ...
      if (outputExceeded) {
        const outputFailure = new Error(
          "Schema-9 runtime command produced excessive output.",
        );
        reject(terminationFailure === undefined
          ? outputFailure
          : new AggregateError(
            [outputFailure, terminationFailure],
            "Schema-9 runtime command output-limit cleanup failed.",
          ));
        return;
      }
      if (launchFailure !== undefined || code !== 0 || exitSignal !== null) {
        ...
        reject(terminationFailure === undefined
          ? commandFailure
          : new AggregateError(
            [commandFailure, terminationFailure],
            "Schema-9 runtime command failed and cleanup verification failed.",
          ));
        return;
      }
...
async function prepareSchema9RuntimeCommandLaunch(
  executable: string,
  arguments_: readonly string[],
  cwd: string,
  environment: NodeJS.ProcessEnv,
  useWindowsSupervisor: boolean,
): Promise<Schema9RuntimeCommandLaunch> {
  if (executable.includes("\0")) {
    throw new TypeError(
      "Schema-9 runtime command executable contains a null byte.",
    );
  }
  if (cwd.includes("\0")) {
    throw new TypeError(
      "Schema-9 runtime command working directory contains a null byte.",
    );
  }
  for (const [index, argument] of arguments_.entries()) {
    if (argument.includes("\0")) {
      throw new TypeError(
        `Schema-9 runtime command argument ${String(index)} contains a null byte.`,
      );
    }
  }
  if (process.platform !== "win32" || !useWindowsSupervisor) {
    return Object.freeze({
      command: executable,
  ...
  const commandLine = [executable, ...arguments_]
  ...
  const targetEnvironment = Object.freeze({
    ...Object.fromEntries(Object.entries(environment).filter(([name]) => (
      !WINDOWS_TRUSTED_ENVIRONMENT_NAMES.has(name.toLocaleLowerCase("en-US"))
    ))),
    SystemRoot: launcher.environment["SystemRoot"],
    WINDIR: launcher.environment["WINDIR"],
    ComSpec: launcher.environment["ComSpec"],
  });
  const environmentBlock = serializeWindowsEnvironmentBlock(targetEnvironment);
  const script = windowsJobSupervisorScript(commandLine, cwd);
  return Object.freeze({
...
function serializeWindowsEnvironmentBlock(
  ...
  for (const [name, value] of Object.entries(environment)) {
    ...
    if (
      ...
      throw new TypeError(
        "Schema-9 runtime command environment is not representable.",
      );
    }
    ...
    if (previous !== undefined) {
      ...
      throw new TypeError(
        `Schema-9 runtime command environment has conflicting ${normalized} entries.`,
      );
    }
  ...
  entries.sort((left, right) => {
    ...
    return byName === 0
      ? left[1].localeCompare(right[1], "en-US")
      : byName;
  });
  const block = `${entries.map((entry) => `${entry[1]}=${entry[2]}`)
    .join("\0")}\0\0`;
...
foreach ($name in $forbiddenHostNames) {
  if ($hostEnvironment.Contains($name)) {
    throw "Schema-9 Engine supervisor inherited a forbidden host variable."
  }
}
foreach ($name in $hostEnvironment.Keys) {
  if ([string]$name -like "COMPlus_*") {
    throw "Schema-9 Engine supervisor inherited a forbidden COMPlus variable."
  }
}
...
public static class Schema9EngineJobSupervisor {
  ...
  private static void ThrowLastError(string operation) {
    throw new Win32Exception(
      ...
      "Schema-9 Engine supervisor could not " + operation + "."
    );
  }
  ...
  public static Int32 Run(
    ...
    try {
      ...
      if (WaitForSingleObject(process.hProcess, INFINITE) != WAIT_OBJECT_0) {
        ThrowLastError("wait for the bounded command");
      }
      UInt32 exitCode;
      ...
      if (!TerminateJobObject(job, 1)) {
        ThrowLastError("terminate residual bounded-command descendants");
      }
      if (WaitForSingleObject(job, JOB_DRAIN_TIMEOUT_MILLISECONDS)
          != WAIT_OBJECT_0) {
        throw new TimeoutException(
          "Schema-9 Engine containment job did not drain after termination."
        );
      }
...
try {
  ...
  exit [Schema9EngineJobSupervisor]::Run(
    $commandLine,
    $workingDirectory,
    $environmentPointer
  )
...
function combineSchema9CleanupFailures(
  first: unknown,
  second: unknown,
  message: string,
): unknown {
...
async function authenticatedWindowsPowerShell(): Promise<AuthenticatedSystemTool> {
  ...
  if (
    ...
    throw new TypeError(
      "Windows PowerShell resolved outside its system directory.",
    );
  }
  const metadata = await lstat(path, { bigint: true });
  if (!metadata.isFile()) {
...
async function assertAuthenticatedSystemToolUnchanged(
  ...
  if (
    ...
    throw new Error("Authenticated Windows system tool changed during use.");
  }
}
