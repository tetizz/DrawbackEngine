...
- `packages/simulation-arena/src/stockfish-agent.ts`
- `packages/simulation-arena/src/async-simulation.ts`
- `packages/simulation-arena/src/simulation.ts`
- `packages/simulation-arena/src/parallel.ts`
- `packages/simulation-arena/src/parallel-worker.ts`
- `packages/probe-search/src/probe-search.ts`
- `data/catalog/observed-drawbacks.json`

Predictor and browser integration requirements are downstream consumer
contracts. They are not implemented inside this engine repository.
...
Get-Content packages/simulation-arena/src/async-simulation.ts
Get-Content packages/probe-search/src/probe-search.ts
rg -n 'new GameSession|legalMoves\(|\.move\(' packages apps/engine-cli/src
```
